...
const SHOP_ADMIN_EMAILS = {
  ...
  'default': ['liyun0420209@gmail.com','popo226353@gmail.com'] 
};
...
const AdminView = ({ setView, orders, products, customers, db, appId, showNotify, appConfig, isDark, toggleTheme }) => {
  ...
  // 👇 1. 新增這個刪除訂單的函式
  const handleDeleteOrder = async (orderId, orderDisplayId) => {
      // 防呆機制：跳出確認視窗，避免手滑誤刪
      if (!window.confirm(`⚠️ 警告：確定要永久刪除訂單 ${orderDisplayId} 嗎？\n此動作無法復原！`)) {
          return;
      }
      
      try {
          await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'orders', orderId));
          showNotify('🗑️ 訂單已永久刪除');
          // 刪除後，如果該訂單原本有被勾選，也要從勾選名單中移除
  ...
  // --- 📝 單筆訂單數量修改 ---
  const handleUpdateOrderQty = async (orderId, newQty) => {
      const qty = parseInt(newQty);
      if (isNaN(qty) || qty <= 0) {
          showNotify('數量必須大於 0', 'error');
          return;
      }
      try {
          await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'orders', orderId), { qty: qty });
          showNotify('數量已更新');
      } catch (e) {
          console.error(e);
          showNotify('更新數量失敗', 'error');
      }
  };
  // 1. 批量刪除函式
const handleBatchDelete = async () => {
    ...
    const confirmMsg = `⚠️ 警告！\n您即將永久刪除這 ${selectedOrderIds.size} 筆訂單。\n刪除後無法復原，確定要執行嗎？`;
    if (!window.confirm(confirmMsg)) return;

    try {
        const promises = Array.from(selectedOrderIds).map(id => 
            deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'orders', id))
        );
        await Promise.all(promises);
        showNotify(`🗑️ 成功刪除 ${selectedOrderIds.size} 筆訂單`);
        setSelectedOrderIds(new Set()); // 清空選取狀態
    } catch (e) {
        console.error(e);
        showNotify('批量刪除失敗', 'error');
    }
};
